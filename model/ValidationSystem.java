...
public class ValidationSystem {
	...
	public ValidationSystem() {
		...
		Scanner scanner = new Scanner(getClass().getResourceAsStream(DataStore.CITIES_FILE));
	...
	public static boolean validateCustPriceEvent(CustPriceChangeEvent event) {
		return (validateTimestamp(event.getDay(), event.getMonth(), event.getYear(), event.getTime())
				&& validateNonEmptyString(event.getEmployee()) && validateOrigin(event.getOrigin())
				&& validateNonEmptyString(event.getDestination()) && (event.getPriority() != null)
	...
	public static boolean validateMailProcessEvent(MailProcessEvent event) {
		return (validateTimestamp(event.getDay(), event.getMonth(), event.getYear(), event.getTime())
				&& validateNonEmptyString(event.getEmployee()) && validateOrigin(event.getOrigin())
				&& validateNonEmptyString(event.getDestination()) && (event.getPriority() != null)
				&& validatePriority(event.getPriority().toString()) && event.getWeight() > 0 && event.getVolume() > 0
				&& event.getRevenue() > 0 && event.getExpenditure() > 0 && event.getDeliveryTime() > 0);
	}
	...
	/**
	 * @param originName
	 * @return true if non-null, non-empty, valid NZ placename, and in correct
	 *         camel-case format
	 */
	...
	public static boolean validateTimestamp(int day, int month, int year, int time) {
		...
		boolean validTime = (year > 0 && month > 0 && month <= 12 && day > 0
